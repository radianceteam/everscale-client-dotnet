...
/*
* TON API version 1.36.1, net module.
...
namespace TonSdk.Modules
{
    ...
    public interface INetModule
    {
        ...
        /// refresh all cached (or visible to user) lists where this sequences presents.
        /// </summary>
        Task<ResultOfSubscribeCollection> SubscribeAsync(ParamsOfSubscribe @params, Func<Newtonsoft.Json.Linq.JToken, int, Task> callback = null);
        ...
        /// 3. Retrieve 20 (size of the page) messages and transactions (3rd layer) and 20*5=100 message ids
        /// (4th layer).
        /// 4. Retrieve the last 5 messages and 5 transactions on the 3rd layer + 15 messages and transactions
        /// (of 100) from the 4th layer
        /// + 25 message ids of the 4th layer + 75 message ids of the 5th layer.
        /// 5. Retrieve 20 more messages and 20 more transactions of the 4th layer + 100 more message ids of the
        /// 5th layer.
        /// 6. Now we have 1+5+20+20+20 = 66 transactions, which is more than 50. Function exits with the tree
        /// of
        /// 1m->1t->5m->5t->25m->25t->35m->35t. If we see any message ids in the last transactions out_msgs,
        /// which don't have
        /// corresponding messages in the function result, it means that the full tree was not received and we
        /// need to continue iteration.
        /// 
        /// To summarize, it is guaranteed that each message in `result.messages` has the corresponding
        /// transaction
    ...
    internal class NetModule : INetModule
    {
        ...
        public async Task<ResultOfSubscribeCollection> SubscribeAsync(ParamsOfSubscribe @params, Func<Newtonsoft.Json.Linq.JToken, int, Task> callback = null)
        {
            return await _client.CallFunctionAsync<ResultOfSubscribeCollection, Newtonsoft.Json.Linq.JToken>("net.subscribe", @params, callback).ConfigureAwait(false);
        }
