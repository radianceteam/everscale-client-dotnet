...
namespace TonSdk.Tests.Modules
{
    public class TvmModuleTests : IDisposable
    {
        ...
        [Fact]
        public async Task Should_Run_Get()
        {
            var result = await _client.Tvm.RunGetAsync(new ParamsOfRunGet
            {
                Account = ElectorAccount,
        ...
        [Fact]
        public async Task Should_Run_Get_With_Input()
        {
            var result = await _client.Tvm.RunGetAsync(new ParamsOfRunGet
            {
                Account = ElectorAccount,
                FunctionName = "compute_returned_stake",
                Input = $"0x{ElectorAddress.Split(":")[1]}"
            });

            Assert.NotNull(result);
            Assert.NotNull(result.Output);
            Assert.Single(result.Output);
            Assert.Equal("[\"0\"]", result.Output.ToString(Formatting.None));
        }

        [Fact]
        public async Task Should_Run_Get2()
        {
            var result = await _client.Tvm.RunGetAsync(new ParamsOfRunGet
            {
                Account = ElectorAccount,
            ...
            Assert.NotNull(result);
            Assert.NotNull(result.Output);
            Assert.Equal("1588268660", result.Output[0]?[0]?[0]?.ToString());
        }

        [EnvDependentFact]
        public async Task Should_Run_Executor()
        {
        ...
        [EnvDependentFact]
        public async Task Should_Run_Tvm()
        {
            await TestRunMessageAsync(async (message, abi, account) =>
            {
                var result = await _client.Tvm.RunTvmAsync(new ParamsOfRunTvm
                {
                    Message = message.Message,
                    Abi = abi,
                    Account = account
        ...
        [Fact]
        public async Task Should_Run_Account_None()
        {
            var result = await _client.Tvm.RunExecutorAsync(new ParamsOfRunExecutor
            {
            ...
            Assert.NotNull(parsed);
            Assert.Equal("0:f18d106c11586689b11e946269ec1550b69654a8d5964de668149c28877fb65a", parsed.Parsed.Value<string>("id"));
            Assert.Equal("Uninit", parsed.Parsed.Value<string>("acc_type_name"));
        }

        [Fact]
        public async Task Should_Run_Account_Uninit()
        {
            var keys = await _client.Crypto.GenerateRandomSignKeysAsync();
            var (abi, tvc) = TestClient.Package("Hello", 2);
            ...
            Assert.NotNull(parsed);
            Assert.Equal(message.Address, parsed.Parsed.Value<string>("id"));
            Assert.Equal("Active", parsed.Parsed.Value<string>("acc_type_name"));
        }

        private async Task TestRunMessageAsync(Func<ResultOfEncodeMessage, Abi, string, Task<string>> run)
        {
            var (abi, tvc) = TestClient.Package("Subscription", 2);
            var keys = await _client.Crypto.GenerateRandomSignKeysAsync();

            var walletAddress = "0:2222222222222222222222222222222222222222222222222222222222222222";
            ...
            var accountJson = await _client.FetchAccountAsync(address);
            ...
            account = await run(message, abi, account);

            message = await _client.Abi.EncodeMessageAsync(new ParamsOfEncodeMessage
            {
                Address = address,
                Abi = abi,
                CallSet = new CallSet
            ...
            Assert.Equal(
                subscribeParams.Value<string>("pubkey"),
                result.Decoded.Output.SelectToken(
                    TestClient.AbiVersion == 1
                    ? "subscription.pubkey"
                    : "value0.pubkey"));
        }
    }
